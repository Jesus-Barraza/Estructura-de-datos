...
// ---------------- Serial: imprimir mapa ----------------

// Convierte la enumeración Direccion a texto legible
...
// Imprime todos los registros del mapa por Serial (del más reciente al más antiguo)
void imprimirMapaSerial() {
  if (map_count == 0 || cabeza == nullptr) {
    Serial.println("Mapa vacío.");
    return;
  }

  Serial.println("---- Registros del mapa (reciente -> antiguo) ----");

  Mapa* nodo = cabeza;
  int impresos = 0;

  while (nodo != nullptr && impresos < map_count) {
    // Imprimir campos principales
    Serial.print("Nro: "); Serial.print(nodo->numero);
    Serial.print(" | IZQ: "); Serial.print(nodo->registro_izq);
    Serial.print(" | MID: "); Serial.print(nodo->registro_cen);
    Serial.print(" | DER: "); Serial.print(nodo->registro_der);
    Serial.print(" | t(ms): "); Serial.print(nodo->timestamp);
    Serial.print(" | Dir: "); Serial.println(direccionToString(nodo->direccion));

    nodo = nodo->siguiente;
    impresos++;
  }

  Serial.println("---- Fin de registros ----");
}

// Llamada desde loop: si llega cualquier texto por Serial, imprimir mapa
void checkSerialAndPrintMap() {
  if (Serial.available() > 0) {
    // Leer toda la línea (o lo que haya) para vaciar el buffer
